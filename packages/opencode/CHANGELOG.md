...
- Enroll newly bound Claustrum OAuth accounts into the routing pool at startup or live without a restart, after exact credential-ID and provider-account verification; persist only secret-free tombstone rows, preserve disabled accounts, and recover missed manifest watch events with one process-shared metadata poll.
